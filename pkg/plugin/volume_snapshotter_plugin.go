...
func (p *NewVolumeSnapshotter) CreateVolumeFromSnapshot(snapshotID, volumeType, volumeAZ string, iops *int64) (string, error) {
	...
	if err != nil {
		p.Errorf("Fail to construct new PE ID from string %s", snapshotID)
		return returnVolumeID, err
	}
	returnPeId, err = p.snapMgr.CreateVolumeFromSnapshot(peId)
	if err != nil {
		p.Errorf("Failed at calling SnapshotManager CreateVolumeFromSnapshot with peId %v, error %v", peId, err)
		return returnVolumeID, err
	}
...
func (p *NewVolumeSnapshotter) CreateSnapshot(volumeID, volumeAZ string, tags map[string]string) (string, error) {
	...
	if err != nil {
		p.Errorf("Fail at calling SnapshotManager CreateSnapshot from peID %v, tags %v", peID, tags)
		return "", err
	}
...
func (p *NewVolumeSnapshotter) DeleteSnapshot(snapshotID string) error {
	...
	peID, err := astrolabe.NewProtectedEntityIDFromString(snapshotID)
	if err != nil {
		p.Errorf("Fail to construct new Protected Entity ID from string %s", snapshotID)
		return err
	}

	err = p.snapMgr.DeleteSnapshot(peID)
	if err != nil {
		p.Errorf("Failed at calling SnapshotManager DeleteSnapshot for peID %v with error %v", peID, err)
		return err
	}
