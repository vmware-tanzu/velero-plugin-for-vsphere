...
func newTestBackupDriverControllerForSecretQueue() *backupDriverController {
	return &backupDriverController{
		logger:      veleroplugintest.NewLogger(),
		secretQueue: workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "test-secret-queue"),
	}
}
...
func TestEnqueueSecret(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			ctrl := newTestBackupDriverControllerForSecretQueue()

			assert.NotPanics(t, func() {
				ctrl.enqueueSecret(test.obj)
			})

			if test.expectEnqueued {
				assert.Equal(t, 1, ctrl.secretQueue.Len())
				key, _ := ctrl.secretQueue.Get()
				assert.Equal(t, test.expectedKey, key)
			} else {
				assert.Equal(t, 0, ctrl.secretQueue.Len())
			}
		})
	}
}
