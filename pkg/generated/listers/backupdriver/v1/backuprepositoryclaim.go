...
type BackupRepositoryClaimLister interface {
	...
	List(selector labels.Selector) (ret []*v1.BackupRepositoryClaim, err error)
...
// List lists all BackupRepositoryClaims in the indexer.
func (s *backupRepositoryClaimLister) List(selector labels.Selector) (ret []*v1.BackupRepositoryClaim, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.BackupRepositoryClaim))
	})
	return ret, err
}
...
type BackupRepositoryClaimNamespaceLister interface {
	...
	Get(name string) (*v1.BackupRepositoryClaim, error)
...
// List lists all BackupRepositoryClaims in the indexer for a given namespace.
func (s backupRepositoryClaimNamespaceLister) List(selector labels.Selector) (ret []*v1.BackupRepositoryClaim, err error) {
...
// Get retrieves the BackupRepositoryClaim from the indexer for a given namespace and name.
func (s backupRepositoryClaimNamespaceLister) Get(name string) (*v1.BackupRepositoryClaim, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("backuprepositoryclaim"), name)
	}
	return obj.(*v1.BackupRepositoryClaim), nil
}
