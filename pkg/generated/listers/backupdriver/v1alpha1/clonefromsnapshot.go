...
// List lists all CloneFromSnapshots in the indexer.
func (s *cloneFromSnapshotLister) List(selector labels.Selector) (ret []*v1alpha1.CloneFromSnapshot, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.CloneFromSnapshot))
	})
	return ret, err
}
...
// List lists all CloneFromSnapshots in the indexer for a given namespace.
func (s cloneFromSnapshotNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.CloneFromSnapshot, err error) {
...
// Get retrieves the CloneFromSnapshot from the indexer for a given namespace and name.
func (s cloneFromSnapshotNamespaceLister) Get(name string) (*v1alpha1.CloneFromSnapshot, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("clonefromsnapshot"), name)
	}
	return obj.(*v1alpha1.CloneFromSnapshot), nil
}
