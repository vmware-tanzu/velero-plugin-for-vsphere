...
// List lists all Uploads in the indexer.
func (s *uploadLister) List(selector labels.Selector) (ret []*v1alpha1.Upload, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Upload))
	})
	return ret, err
}
...
// List lists all Uploads in the indexer for a given namespace.
func (s uploadNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Upload, err error) {
...
// Get retrieves the Upload from the indexer for a given namespace and name.
func (s uploadNamespaceLister) Get(name string) (*v1alpha1.Upload, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("upload"), name)
	}
	return obj.(*v1alpha1.Upload), nil
}
